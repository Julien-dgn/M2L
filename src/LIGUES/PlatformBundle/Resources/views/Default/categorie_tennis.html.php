	...
	<body>									
		<section id= "categorieMembre">		<!-- CODE -->
			<form method= "post" action= "<?php $url = $this->get('router')->generate('ligues_platform_forum_espace_membre_ajouter_article_formulaire', array(),true); echo $url; ?>">
					...
					-- PAGE: TENNIS -- Ajouter un article dans la catégorie: 
						<select name= "article_foot" class= "select"><option id = "foot">TENNIS</option></select> 
			...
			<form method= "post" action= "<?php $url = $this->get('router')->generate('ligues_platform_forum_espace_membre_voir_article', array(),true); echo $url; ?>">
					<section id= "affiche_article_foot">
						...
						$nombreLimitV= ', ';
						$nombreLimit2= 4;
						$requete= 'SELECT id, titre_article, categorie_article, message FROM article WHERE categorie_article = "TENNIS" LIMIT ';
			...
			<form method= "post" action= "<?php $url = $this->get('router')->generate('ligues_platform_forum_espace_membre_categorie_tennis', array(),true); echo $url; ?>">	
							 	Page: <?php 
